...
class ContextCompressor:
    ...
    def _generate_summary(self, turns_to_summarize: List[Dict[str, Any]]) -> Optional[str]:
        ...
        for msg in turns_to_summarize:
            ...
            # Handle Anthropic content blocks (multimodal tool results, e.g. computer_use screenshots).
            # Prune large image blocks to just their text_summary to save tokens.
            if isinstance(content, list):
                pruned = []
                for block in content:
                    if isinstance(block, dict) and block.get("type") == "image":
                        text = block.get("source", {}).get("text_summary", "")
                        pruned.append({"type": "text", "text": text or "[image data]"})
                    else:
                        pruned.append(block)
                content = "\n".join(
