...
class AIAgent:
    ...
    def _anthropic_messages_create(self, api_kwargs: dict):
        ...
        # Native Anthropic tool types (e.g. computer_20251124) must be sent
        # via client.beta.messages.create() — the standard messages.create()
        # rejects them with 'unhashable type: slice'.
        tools = api_kwargs.get("tools", [])
        has_native_tool = any(
            isinstance(t, dict) and t.get("type", "").startswith("computer")
            for t in tools
        )
        if has_native_tool:
